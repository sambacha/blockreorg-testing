    ...
    def parse_file(self, file):
        ...
        with open(file, "r") as f:
            for line in f:
                ...
                drop.append(
                    stats["network"]["eth1"]["ingress"]["dropped"]
                    + stats["network"]["eth1"]["egress"]["dropped"]
                )
                err.append(
                    stats["network"]["eth1"]["ingress"]["errors"]
                    + stats["network"]["eth1"]["egress"]["errors"]
                )
                # cnt += 1

        sorted_stats = sorted(
            zip(time, cpu, mem, egress, ingress, drop, err), key=lambda item: item[0]
        )
        ...
        resources = {
            ...
            "cpuSum": cpu,
            "memory": mem,
            "net": {"egress": egress, "ingress": ingress,},
            "pkt": {"drop": drop, "err": err},
        }
